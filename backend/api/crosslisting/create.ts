...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (missingCredentials.length > 0) {
      throw new ValidationError(`Missing credentials for platforms: ${missingCredentials.join(', ')}`);
    }
    ...
    const limit = crossListingLimits[userSubscription.subscription_tier as keyof typeof crossListingLimits];
    if ((monthlyRequests || 0) >= limit) {
      throw new ValidationError(`Monthly cross-listing limit reached (${limit}). Upgrade your subscription for more.`);
    }
    ...
    if (itemsError || !userItems || userItems.length !== inventoryItems.length) {
      throw new ValidationError('Some inventory items not found or not accessible');
    }
    ...
    if (invalidItems.length > 0) {
      throw new ValidationError(`Cannot cross-list items with status: ${invalidItems.map(i => i.status).join(', ')}`);
    }
...
async function processCrossListingRequest(requestId: string) {
  ...
  for (const itemId of request.inventory_items) {
    ...
    for (const platform of request.target_platforms) {
      try {
        ...
        const { data: marketplaceListing, error: listingError } = await supabaseAdmin
...
async function simulatePlatformListing(item: any, platform: string, optimizeSEO: boolean) {
...
function optimizeTitle(title: string, platform: string): string {
...
function optimizeDescription(description: string, platform: string): string {
  ...
  const tags = platformTags[platform as keyof typeof platformTags] || '';
  return `${description}\n\n${tags}`;
}
