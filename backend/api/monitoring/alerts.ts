...
import { AlertManager, alertingConfig } from '../../../monitoring/alerting.config';
  ...
  res.status(200).json({
    ...
    data: {
      ...
      config: {
        ...
        channels: Object.keys(alertingConfig.channels).filter(
...
async function handleTriggerAlert(req: VercelRequest, res: VercelResponse) {
  ...
  const rule = alertingConfig.rules.find(r => r.id === ruleId);
  if (!rule) {
...
function checkActiveAlerts(metrics: any) {
  const activeAlerts = [];
...
function getThresholdForRule(rule: any) {
...
function getAlertsFromHistory(filters: any) {
...
function isAuthorizedForWrite(req: VercelRequest): boolean {
  const authHeader = req.headers.authorization;
