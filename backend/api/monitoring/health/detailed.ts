...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const [dbHealth, servicesHealth, systemHealth, securityHealth, depsHealth] = healthChecks.map(
      result => result.status === 'fulfilled' ? result.value : { status: 'fail', error: result.reason?.message }
    );
...
function checkRequiredSecrets(): boolean {
...
function calculateDetailedHealthScore(checks: any): number {
  let score = 100;
  
  // Database health (30% weight)
  if (!checks.database.status) score -= 30;
  ...
  if (checks.system.status !== 'pass') score -= 20;
  
  // Security (15% weight)
  if (checks.security.status !== 'pass') score -= 15;
  
  // Dependencies (10% weight)
  if (checks.dependencies.status !== 'pass') score -= 10;
