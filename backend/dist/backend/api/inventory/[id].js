...
async function getInventoryItem(id, userId, res) {
    const { data: item, error } = await database_1.supabaseAdmin
...
async function updateInventoryItem(id, userId, updates, res) {
    // Validate the item exists and belongs to user
    const { data: existingItem, error: fetchError } = await database_1.supabaseAdmin
        .from('inventory_items')
        .select('*')
        .eq('id', id)
        .eq('user_id', userId)
    ...
    if (Object.keys(updateData).length === 0) {
        throw new errorHandler_1.ValidationError('No valid fields to update');
    }
    // Validate SKU uniqueness if being updated
    if (updateData.sku && updateData.sku !== existingItem.sku) {
        const { data: skuCheck } = await database_1.supabaseAdmin
            .from('inventory_items')
            .select('id')
            .eq('user_id', userId)
            ...
            .single();
        if (skuCheck) {
            throw new errorHandler_1.ValidationError('SKU already exists for another item');
        }
    }
    ...
    // Update the item
    const { data: updatedItem, error: updateError } = await database_1.supabaseAdmin
        .from('inventory_items')
        .update(updateData)
        .eq('id', id)
        .eq('user_id', userId)
    ...
    if (updateData.quantity_available !== undefined) {
        try {
            await database_1.supabaseAdmin.rpc('sync_inventory_across_platforms', {
...
async function deleteInventoryItem(id, userId, res) {
    // Check if item has active listings
    const { data: activeListings } = await database_1.supabaseAdmin
    ...
    const { error: deleteError } = await database_1.supabaseAdmin
        .from('inventory_items')
        .delete()
        .eq('id', id)
        .eq('user_id', userId);
