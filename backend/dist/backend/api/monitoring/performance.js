...
async function handler(req, res) {
    ...
    try {
        ...
        const performanceData = {
            ...
            api: apiPerformance,
            system: systemPerformance,
            thresholds: performance_config_1.performanceConfig.thresholds
        };
...
async function testDatabasePerformance() {
    ...
    try {
        ...
        const thresholds = performance_config_1.performanceConfig.thresholds.database;
        ...
        return {
            ...
            details: {
                connectionTime: -1,
                executionTime: -1,
                thresholds: performance_config_1.performanceConfig.thresholds.database
            }
        };
    }
}
async function testApiPerformance(req) {
    ...
    try {
        ...
        const result = performanceMonitor.endTimer(timerId);
        const thresholds = performance_config_1.performanceConfig.thresholds.api;
        ...
        return {
            ...
            endpoints: {},
            details: {
                thresholds: performance_config_1.performanceConfig.thresholds.api,
                region: 'unknown'
            }
        };
    }
...
function getSystemPerformance() {
    ...
    const thresholds = performance_config_1.performanceConfig.thresholds.memory;
...
function calculatePerformanceScore(metrics) {
    ...
    if (metrics.database.level === 'critical')
        score -= 40;
    else if (metrics.database.level === 'slow')
        score -= 20;
    else if (metrics.database.level === 'acceptable')
        score -= 10;
    // API performance impact (35% weight)
    if (metrics.api.level === 'critical')
        score -= 35;
    else if (metrics.api.level === 'slow')
        score -= 18;
    else if (metrics.api.level === 'acceptable')
        score -= 9;
    // System performance impact (25% weight)
    if (metrics.system.memory.level === 'critical')
        score -= 25;
    else if (metrics.system.memory.level === 'warning')
        score -= 12;
