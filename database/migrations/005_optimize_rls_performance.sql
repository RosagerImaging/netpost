-- =====================================================================
-- MIGRATION 005: Optimize RLS Performance
-- =====================================================================
-- This migration optimizes RLS policies to improve performance by preventing
-- auth function re-evaluation for each row. Replaces auth.uid() with (select auth.uid())
-- 
-- Performance Issue: The Supabase performance advisor identified that RLS policies
-- were using auth.uid() calls that re-evaluate for each row, causing suboptimal
-- query performance at scale.
-- 
-- Solution: Wrap auth.uid() calls with SELECT to evaluate once per query instead
-- of once per row. This is the recommended approach per Supabase documentation.
--
-- Affected Tables:
...
-- =====================================================================
-- All RLS policies have been optimized to use (select auth.uid()) instead
-- of direct auth.uid() calls. This prevents the auth function from being
-- re-evaluated for each row, significantly improving query performance.
--
-- Changes Made:
-- - 10 RLS policies optimized across all user-scoped tables
-- - Auth function evaluation reduced from once-per-row to once-per-query
-- - No functional changes to security model - same access patterns preserved
-- =====================================================================